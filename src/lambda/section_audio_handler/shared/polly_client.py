...
MAX_CHARS_PER_REQUEST = 6000  # Polly limit is 6000 total, 3000 billable
SAFE_CHUNK_SIZE = 5500  # Leave some margin for safety
...
def chunk_text_for_polly(
    ...
    Args:
        text: Text to split
        max_chars: Maximum characters per chunk (default: 5500)
    ...
    while current_pos < len(clean_text):
        ...
        end_pos = min(current_pos + max_chars, len(clean_text))
        chunk = clean_text[current_pos:end_pos]
        ...
        while current_pos < len(clean_text) and clean_text[current_pos] in ' \t\n':
            current_pos += 1
    
    logger.info(f"Split {len(clean_text)} chars into {len(chunks)} chunks")
    return chunks
...
def get_speech_marks(
    ...
    logger.info(f"Getting speech marks: {len(clean_text)} chars, types={mark_types}")
...
def merge_speech_marks(
    ...
    logger.info(f"Merged {len(merged)} speech marks from {len(marks_results)} chunks")
    return merged
...
def synthesize_lecture_with_marks(
    ...
    for i, (chunk_text, byte_offset) in enumerate(chunks):
        ...
        # More accurate: decode and measure, but this is faster
        # MP3 at 24kHz mono ~= 3KB per second
        estimated_duration_ms = int(len(audio_result.audio_bytes) / 3 * 1000 / 1024)
        audio_durations.append(estimated_duration_ms)
