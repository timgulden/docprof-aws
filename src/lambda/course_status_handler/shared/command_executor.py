    ...
    try:
        ...
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                logger.info(f"Database stats: {chunk_stats['total']} total chunks, {chunk_stats['with_embedding']} with embeddings")
                ...
                matching_chunks_count = cur.fetchone()['matching_count']
...
def _assign_chunks_to_sections(sections: List) -> List:
    """
    Assign relevant chunks to each section based on title and learning objectives.
    
    This ensures sections have source material for lecture generation.
    Searches for 10 most relevant chunks per section.
    """
    from shared.db_utils import get_db_connection
    from shared.bedrock_client import generate_embeddings
    from psycopg2.extras import RealDictCursor
    
    # Skip if all sections already have chunk_ids
    sections_needing_chunks = [s for s in sections if not s.chunk_ids]
    if not sections_needing_chunks:
        logger.info("All sections already have chunk_ids assigned")
        return sections
    
    logger.info(f"Assigning chunks to {len(sections_needing_chunks)} sections without chunk_ids")
    
    # Create a mapping to update sections
    updated_sections = []
        ...
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                for section in sections:
                    # If section already has chunks, keep them
                    if section.chunk_ids:
                        updated_sections.append(section)
                        continue
                    
                    # Build search text from title and objectives
                    search_text = section.title
                    if section.learning_objectives:
                        search_text += ". " + ". ".join(section.learning_objectives)
                    if section.content_summary:
                        search_text += ". " + section.content_summary
                    
                    logger.info(f"Searching chunks for section: {section.title[:50]}...")
                    
                    try:
                        # Generate embedding for section content
                        embeddings = generate_embeddings([search_text])
                        if not embeddings or not embeddings[0]:
                            logger.warning(f"Failed to generate embedding for section: {section.title}")
                            updated_sections.append(section)
                            continue
                        
                        query_embedding = embeddings[0]
                        
                        # Search for relevant chunks - get 10 chunks per section
                        # This provides broader source coverage for lectures
                        cur.execute("""
                            SELECT 
                                chunk_id,
                                chunk_type,
                        ...
                        logger.info(f"  Found {len(chunk_ids)} chunks for '{section.title[:30]}' (min_sim=0.3)")
                        
                        # Create updated section with chunk_ids
                        updated_section = section.model_copy(
                            update={'chunk_ids': chunk_ids}
                        )
                        updated_sections.append(updated_section)
                        
                    except Exception as e:
                        logger.error(f"Error finding chunks for section '{section.title}': {e}")
                        updated_sections.append(section)  # Keep original
    
    except Exception as e:
        logger.error(f"Error in _assign_chunks_to_sections: {e}", exc_info=True)
        return sections  # Return original sections on error
    
    # Log summary
    total_chunks = sum(len(s.chunk_ids) for s in updated_sections)
    logger.info(f"✓ Assigned total of {total_chunks} chunks across {len(updated_sections)} sections")
    
    return updated_sections


def execute_create_sections_command(command: CreateSectionsCommand) -> Dict[str, Any]:
    """Execute CreateSectionsCommand - store sections in database with auto-assigned chunks."""
    try:
        ...
        # Auto-assign chunks to sections that don't have them
        sections_with_chunks = _assign_chunks_to_sections(sections)
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                execute_values(
                    ...
                    values,
                    template="""(
                        %s::uuid, %s::uuid, %s::uuid, %s, %s, %s, %s, %s, %s::uuid[], %s, %s, %s, %s, %s
                    )"""
                )
